...
const Card = ({ blogs }) => {
  ...
  return (
    <div className="text-left lg:flex flex-grow w-full space-y-5 rounded-2xl dark:bg-gray-800 shadow-lg outline outline-1 outline-white dark:outline-gray-500 bg-gray-200 dark:text-gray-400 dark:hover:text-white">
      <figure>
        <img className="md:h-[340px] md:w-[820px] rounded-2xl" src={blogs?.image} alt="image" />
      </figure>
      <div className="w-full space-y-5 lg:p-6 p-8">
        <div className="w-full space-y-4">
          <div>
            <Link to={blogs?.category} className="text-sm lg:text-base shadow lg:font-medium hover:bg-gray-100 dark:bg-gray-800 dark:text-gray-50 outline outline-white dark:hover:bg-gray-500 dark:hover:text-black  hover:text-black text-gray-600 hover:outline outline-1 rounded-lg w-fit px-4 py-2 normal-case my-4">
              {toNormalCase(blogs?.category)}
            </Link>
          </div>
          <h2 id="link" className="md:text-2xl text-gray-500 hover text-xl md:font-semibold dark:text-white dark:hover:text-gray-500">
            <Link
          ...
          <h2 className="text-sm md:text-lg font-medium my-2 dark:text-white">{blogs?.shortDesc}</h2>
          <div className=" text-gray-500 text-sm dark:text-white">
            <span className="inline-flex gap-1"><FaCalendar className="text-gray-400 text-xs md:text-base" />{formatDate(blogs?.date)}</span>
            <p>By <span className="uppercase  text-xs md:text-base font-semibold">{blogs?.author}</span></p>
          </div>
        </div>
        <div>
          {
            accessData ?
              <div className="grid grid-cols-3 gap-5">
                <div>
                  <LoadingButton className="md:text-lg text-sm font-medium w-24" to={`/blog/details/${blogs?._id}`} btnLabel={`Details`} />
                </div>
                <div>
                  <LoadingButton onClick={handleWishlist} className="md:text-lg text-sm font-medium w-24" btnLabel={`Wishlist`} />
                </div>
                <div>
                  <LoadingButton className="md:text-lg text-sm font-medium w-24" to={`/blog/update/${blogs?._id}`} btnLabel={`Update`} />
                </div>
              </div>
              :
              <div>
